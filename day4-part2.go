...
func main() {
	...
	C.print_int(C.CString("removed rolls: "), C.int(removedTotal))
}

func removalPass(rows [][]bool) int {
	var removed int
	...
	for row, _ := range rows {
		for col, _ := range rows[row] {
			...
			if remaining > 0 {
				// we remove immediately so we can get more worth for our pass :)
				rows[row][col] = false
				removed++
			}
		}
